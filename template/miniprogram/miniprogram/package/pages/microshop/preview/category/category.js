...
Page({
  ...
  onLoad: function(options) {
    ...
    if (options.shopkeeper_id) {
      this.setData({
        shopkeeper_id: options.shopkeeper_id
      })
    }
  ...
  getCategoryList: function() {
    ...
    re.request(api.get_micPreviewShopCategory, postData, header).then((res) => {
      wx.hideLoading();
      that.setData({
  ...
  onGoodsListPage: function(e) {
    ...
    wx.navigateTo({
      url: '../list/list?category_id=' + category_id + '&category_name=' + category_name + '&shopkeeper_id=' + that.data.shopkeeper_id,
    })
  }
