<template>
  <div class="thingcircle-fans">
    ...
    <List
      ...
      :empty="{message: '你还没有关注任何人~', showFoot: false}"
      @load="loadList"
    >
      <div class="cell" v-for="(item,index) in list" :key="index">
        <img :src="item.auser_headimg" :onerror="$ERRORPIC.noAvatar" class="img" />
        <div class="info">
          <span>{{item.attention_thing_user_name ? item.attention_thing_user_name : "匿名"}}</span>
        ...
        <div class="btn" v-if="item.mutual">
          <van-button
            ...
            type="danger"
            size="small"
            @click="sensitiveOthers(item.follow_uid,index)"
          >互相关注</van-button>
        </div>
...
export default sfc({
  ...
  methods: {
    loadList(init) {
      const $this = this;
      GET_ATTENTIONUSERLIST($this.params).then(({ data }) => {
        let list = data.data;
