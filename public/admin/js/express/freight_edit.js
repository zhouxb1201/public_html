$(function () {
    $("#freight_template_from").validate();
    // 弹出框
    ...
    function showSelectAreaModal(e) {
        layer.open({
            ...
            btn1: function (index, layero) {
                ...
                $('#' + shipping_fee_obj).find(".js-region-info").html(getAreas());
                new_one = false;
            },
    ...
    $("#add-area").click(function () {
        var html = "<tr id='shipping_fee_area_id_new_" + new_index + "' data-shipping-fee-area-id='new_" + new_index + "'>" +
            ...
            "<a class='js-select-city' href='javascript:;'><i class='edit icon-edit'></i>编辑</a>" +
            "</td>" +
            "<td class='tdWidth'>" +
            "<input type='text' class='table_input' name='main_level_num' placeholder='0' data-msg='首件不能是负数、并且必须是整数' data-rule='int' required title='请填写该项'/>" +
            "</td>" +
            "<td class='tdWidth'>" +
            "<input type='text' class='table_input' name='main_level_fee' placeholder='0' data-msg='首件运费不能为负数，并且保留两位小数' required title='请填写该项'/>" +
            "</td>" +
            "<td class='tdWidth'>" +
            "<input type='text' class='table_input' name='extra_level_num' placeholder='0' data-msg='续件不能是负数、并且必须是整数' data-rule='greater_int' required title='请填写该项'/>" +
            "</td>" +
            "<td class='tdWidth'>" +
            "<input type='text' class='table_input' name='per_extra_level_fee' placeholder='0' data-msg='续件运费不能为负数，并且保留两位小数' data-rule='greater_double' required title='请填写该项'/>" +
            "</td>" +
            "<td>" +
            "<div class='delete-area del'>删除</div>" +
            "</td>" +
            "</tr>";
            ...
            success: function (res) {
                if (parseInt(res.code)) {
                    layer.msg(res.message, {icon: 1, time: 1000}, function () {
                        window.location.href = __URL(ADMINMAIN + '/Express/freightTemplateList?co_id=' + shipping_fee_detail.co_id);
                    })
                } else {
...
function buildData() {
    ...
    $("tr[id^=shipping_fee_area_id_]").each(function () {
        ...
        if (shipping_fee_detail['shipping_area'][curr_shipping_fee_area_id]) {
            shipping_fee_detail['shipping_area'][curr_shipping_fee_area_id]['main_level_num'] = $(this).find("input[name=main_level_num]").val();
            shipping_fee_detail['shipping_area'][curr_shipping_fee_area_id]['main_level_fee'] = $(this).find("input[name=main_level_fee]").val();
            shipping_fee_detail['shipping_area'][curr_shipping_fee_area_id]['extra_level_num'] = $(this).find("input[name=extra_level_num]").val();
            shipping_fee_detail['shipping_area'][curr_shipping_fee_area_id]['per_extra_level_fee'] = $(this).find("input[name=per_extra_level_fee]").val();
