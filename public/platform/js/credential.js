define(['util', 'jquery-ui'], function (util) {
	...
	modal.initDefaultItems = function(){
		$.each(modal.items, function (itemid, item) {
			if(modal.type == 2 && item.id == 'shop_head' && (!item.params || !item.params.styletype)){
				item.params = {}
				item.params.styletype = '1'
			}
			if(modal.type == 5 && item.id == 'commission_fixed' && (!item.params || !item.params.styletype)){
				item.params = {}
				item.style = {}
				item.params.styletype = '1'
	...
	modal.initNavs = function () {
		...
		var navgroup = {
			...
			1: ['search', 'menu', 'notice', 'banner', 'picturew', 'listmenu'],
			2: ['search', 'menu', 'notice', 'banner', 'picturew'],
			3: [],
			4: ['menu', 'notice', 'listmenu'],
			5: ['menu', 'notice', 'listmenu'],
			6: ['search', 'menu', 'notice','banner', 'picturew', 'listmenu'],
			7: [],
			8: [],
