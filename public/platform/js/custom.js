define(['util', 'jquery-ui'], function (util) {
	...
	modal.init = function (params) {
		...
		modal.initNavs();
		modal.initSortable();
		$("#page,#wechat").unbind('click').click(function () {
	...
	modal.initData = function (data) {
		...
		modal.wechat_set = params.wechat_set;
	...
	modal.initPage = function (initE) {
		...
		if (!modal.wechat_set) {
			modal.wechat_set = modal.defaultWechat()
		}
		modal.initWechat()
			...
			$.each(modal.items, function (itemid, item) {
				if(item.id == 'detail_fixed'){
					...
					$.each(modal.defaultDetailItems(),function(detail_itemid,detail_item){
						...
						// 没有店铺应用不显示店铺组件
						if(!modal.addonsIsUse.shop&&detail_item.id=='detail_shop'){
							delete newItems[detail_itemid];
						}
					})
				}else{
					newItems[itemid] = item;
	...
	// 初始化公众号组件
	modal.initWechat = function () {
		if (modal.wechat_set.is_show == '1') {
			$("#wechat").html(tpl("tpl_show_wechat", modal.wechat_set));
		} else {
			$("#wechat").empty()
		}
	};
	...
	modal.initEditor = function (scroll) {
		...
		$('[data-toggle="selectUrl"]').unbind('click').click(function () {
			...
			},'wap&template_type=' + modal.type)//by sgw 为了判断B端的店铺首页type（用来带自营店shop_id=0链接）
		})
	...
	modal.initNavs = function () {
		...
		var navgroup = {
			...
			1: ['search', 'menu', 'notice', 'banner', 'picturew', 'popupAdv','goodsCps'],
			2: ['search', 'menu', 'notice', 'banner', 'picturew'],
			3: ['detail_banner','detail_info','detail_promote','detail_specs','detail_service','detail_delivery','detail_desc'],
			4: ['menu', 'notice'],
			5: ['menu', 'notice'],
	   ...
	   if (modal.type == 9) {
		...
		if (modal.type == 3 && modal.addonsIsUse.shop) {
			navgroup[modal.type].push('detail_shop')
		}
	...
	modal.save = function (preview) {
		...
		modal.data = {};
		modal.data.page = modal.page
		modal.data.items = modal.items
		if (preview) {
			...
			$.dialog({
				...
				content: '<iframe id="wap-iframe" style="width:100%;height:600px;border:1px solid #e8e8e8;" src="' + modal.realm_ip + '/wap/preview?t='+(new Date()).getTime()+'"></iframe>',
				bgOpacity: 0.1,
		...
		$.ajax({
			type: 'post',
			url: __URL(PLATFORMMAIN + '/config/addCustomTemplate'),
			data: {
				'id': modal.id,
				'template_data': modal.data,
				'tabbar': modal.tabbar,
				'copyright': modal.copyright,
				'wechat_set': modal.wechat_set,
				'popupAdv':modal.popupAdv
			},
	...
	// 格式化items对象数据
	modal.itemsObjtoArr = function(_items){
		if($.isArray(_items)){
			return _items
		}
		...
		$.each(items,function(key,item){
			...
			if(!util.isEmpty(item.data)){
				$.each(item.data,function(data_key,data_item){
					data_item['data_key'] = data_key
					obj.data.push(data_item)
				})
			}
			arr.push(obj)
		})
	...
	// 格式化items数组数据
	modal.itemsArrtoObj = function(_items){
		if(!$.isArray(_items)){
			return _items
		}
		var items = $.extend(true, {},_items)
		...
		$.each(items,function(i,item){
			obj[item.item_key] = {}
			$.each(item,function(item_key,item_item){
				if(item_key == 'data'){
					if(!util.isEmpty(item_item)){
						obj[item.item_key]['data'] = {}
						$.each(item_item,function(data_key,data_item){
							obj[item.item_key]['data'][data_item.data_key] = data_item
						})
					}
				}else{
					obj[item.item_key][item_key] = item_item
				}
			})
		})
		return obj;
	...
	// 默认wechat_set数据
	modal.defaultWechat = function () {
		return {
			is_show: '0',
			default_title1: '您还没有关注公众号！',
			default_title2: '关注公众号，带你轻松玩转商城。',
			invite_title1: '',
			invite_title2: '',
			btn_text: '立即关注',
			btn_action: '1',
