	...
	*/
	...
	public function execute($request, $authToken = null, $appInfoAuthtoken = null) {
		...
		if (false === $respWellFormed) {
                    echo 'HTTP_RESPONSE_NOT_WELL_FORMED';
    ...
    function verifyKeys($post,$PUB_KEY,$key,$signStrs)
    {
        $signStrs = base64_decode($signStrs);
        //读取公钥文件
        $pubKey = formatPubKey($PUB_KEY);
        // //转换为openssl格式密钥
        $res = openssl_get_publickey($pubKey);
        // 生成摘要
        $data['key'] = $key;
        $data['appUID'] = $post['appUID'];
        $data['outTradeNO'] = $post['outTradeNO'];
        $data['body'] = '商品描述';
        $data['ext'] = $post['ext']; // 额外参数
        $data['amount'] = $post['amount']; //
        $data['subject'] = '标题';
        $data['notifyUrl'] = urlencode($post['notifyUrl']);
        $data['buyAddress'] = $post['buyAddress'];// 购买者币种账号地址
        $data['currenecyType'] = $post['currenecyType'];
        $data['orderType'] = $post['orderType']; // 订单类型 0 充值 2 购物
        $data = createSignLinkstring($data);
        $result = (bool)openssl_verify($data, $signStrs, $res, OPENSSL_ALGO_SHA256);
        openssl_free_key($res);
        return $result;
    }
