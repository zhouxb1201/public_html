...
class WxPayApi
{
    ...
    public static function unifiedOrderApp($inputObj, $timeOut = 6)
    {
        ...
        if (! $inputObj->IsOut_trade_noSet()) {
            ...
            $wchat_config = $config->getWpayConfigApp($website_id);
    ...
    public static function unifiedOrderMir($inputObj, $timeOut = 6, $website_id = 0)
    {
        ...
        $wchat_config = $config->getWpayConfigMir($website_id);
...
//        $result1 = json_encode($result);
    ...
    public static function unifiedOrders($inputObj, $timeOut = 6)
    {
        ...
        $config = new config();
        $WxPayConfig = $config->getWpayConfigs()['value'];
        $inputObj->SetAppid($WxPayConfig['appid']); // 公众账号ID
    ...
    private static function postXmlCurl($xml, $url, $useCert = false, $second = 30,$website_id=0)
    {
        ...
        if ($useCert == true) {
            ...
            curl_setopt($ch, CURLOPT_SSLCERT, $_SERVER['DOCUMENT_ROOT'].'/upload/'.$website_id.'/mp/apiclient_cert.pem');
            curl_setopt($ch, CURLOPT_SSLKEYTYPE, 'PEM');
            curl_setopt($ch, CURLOPT_SSLKEY, $_SERVER['DOCUMENT_ROOT'].'/upload/'.$website_id.'/mp/apiclient_key.pem');
        }
        // post提交方式
