...
//        if (isset($config['expire']) && strstr($_SERVER['QUERY_STRING'], '/platform')) {
...
//        }
     ...
     * @param int $expire 多久之后过期,单位秒
     * @param string|null $prefix 作用域（前缀）
     * @return void
     */
    public static function set($name, $value = '', $expire = 0, $prefix = null)
    {
        ...
        if (strpos($name, '.')) {
            ...
            if ($prefix) {
                ...
                if ($expire > 0) {
                    $_SESSION[$prefix][$name1][$name2 . 'expire'] = time() + $expire;
                }
            } else {
                $_SESSION[$name1] = session::get($name1)?:array();
                $_SESSION[$name1][$name2] = $value;
                if ($expire > 0) {
                    $_SESSION[$name1][$name2 . 'expire'] = time() + $expire;
                }
            }
            ...
            if ($expire > 0) {
                $_SESSION[$prefix][$name . 'expire'] = time() + $expire;
            }
        } else {
            $_SESSION[$name] = $value;
            if ($expire > 0) {
    ...
    public static function get($name = '', $prefix = null)
    {
        ...
        if ('' == $name) {
            ...
            if (strpos($name, '.')) {
                list($name1, $name2) = explode('.', $name);
                if (isset($_SESSION[$prefix][$name1][$name2 . 'expire']) && $_SESSION[$prefix][$name1][$name2 . 'expire'] < time()) {
                    Session::delete($name);
                    return null;
                }
                $value = isset($_SESSION[$prefix][$name1][$name2]) ? $_SESSION[$prefix][$name1][$name2] : null;
            } else {
                if (isset($_SESSION[$prefix][$name . 'expire']) && $_SESSION[$prefix][$name . 'expire'] < time()) {
                    Session::delete($name);
                    return null;
                }
            ...
            if (strpos($name, '.')) {
                list($name1, $name2) = explode('.', $name);
                if (isset($_SESSION[$name1][$name2 . 'expire']) && $_SESSION[$name1][$name2 . 'expire'] < time()) {
                    Session::delete($name);
                    return null;
                }
